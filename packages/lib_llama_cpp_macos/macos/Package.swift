...
let prebuiltFrameworkPath = packageRoot + "/lib_llama_cpp_macos/Frameworks/lib_llama_cpp_macos.xcframework"
...
if usePrebuilt {
    target = .binaryTarget(
        ...
        path: "lib_llama_cpp_macos/Frameworks/lib_llama_cpp_macos.xcframework"
    )
} else if hasSources {
    target = .target(
        name: "lib_llama_cpp_macos",
